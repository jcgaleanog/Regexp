...
def random_cadenabits(num_bits)
  ...
  # puts "Prueba de Funcionamiento: "+valor.to_s
  return valor
end
...
def decode_Cromosoma(cadenabits, codon)
  ...
  puts "Valor del Cromosoma en bit: "+cadenabits.to_s
  puts "Valor del Cromosoma: "+ints.to_s
...
def generarExp(gramaticaE, valEntero, max_pro)
  ...
  begin
    ...
    gramaticaE.keys.each do |key|
        symbolic_string = symbolic_string.gsub(key) do |k|
          ...
          #set = ((k=="Aval1" && depth>=max_pro-1) || (k=="Fval2" && depth>=max_pro-1) || (k=="Hval3" && depth>=max_pro-1))  ? gramaticaE["Gval1"] : gramaticaE[k]
          set = ((k=="Aval1" && depth>=max_pro-1) || (k=="Fval2" && depth>=max_pro-1))  ? gramaticaE["Gval1"] : gramaticaE[k]
          puts "Valor de K:"+k.to_s
          puts "Valor de gramatica k:"+gramaticaE[k].to_s
          puts "Valor de max_pro:"+max_pro.to_s
          puts "Valor de profundida:"+depth.to_s
          puts "Valor set:"+set.to_s         
          generado = valEntero[offset].modulo(set.size)
          puts "Valor Interger : "+generado.to_s
          puts "Valor Intergers : "+valEntero[offset].to_s
          offset = (offset==valEntero.size-1) ? 0 : offset+1
          puts "Valor offset : "+offset.to_s
          $generaHtml<< '<tr><td>'<< cont3.to_s << '</td><td>' << symbolic_string.to_s << '</td></tr>'
          puts "Valor interger set : "+set[generado].to_s
  ...
  puts "Valor simbol final:"+symbolic_string
...
valorBuscar=""
  ...
  #Valor de la busqueda en la expresion
#  valorBuscar="A??B"
  return valorBuscar
end
...
def cargar_archivo_p()
  # archivo = File.new("pruebas.txt","r")
...
def resultadosMostrados(program, pruebasCromo=1)
  ...
  pruebasCromo.times do
    ...
    rangoCar=""
    #if (/Caracteres/.match(program)=="Caracteres") 
    if (program.strip.include?("Caracteres")) 
      #$generaHtml<< '<tr><td>Entro 2 ' << '</td></tr>'
    ...
    rangoNum=""
    #if (program.match("num1 ")=="num1 ") || (program.match("num2 ")=="num2 ")
    if (program.strip.include?("num1")) || (program.strip.include?("num2"))
       rangoNum=rangoNumeros()
       #$generaHtml<< '<tr><td>Entro 3 ' << '</td></tr>'
   ...
   # $generaHtml3 << '<tr><td>Rendimiento en tamaño de la derivacion: ' << program.size.to_s << '</td></tr>'
    ...
    #sum_error += (puntos - funcionPrueba(textoBus,rangoCar)).abs
    sum_error += (puntos+tamExp.to_f)
    result=puntos.to_s
    #$generaHtm3<< '<tr><td>Valor Total de diferencia de Error: ' << sum_error.to_s << '</td></tr>' << '</table>'
...
def valorAptitud(program, pruebasCromo=5)
  ...
  pruebasCromo.times do
    ...
    puts ""
    puts "El numero de Pruebas es:"+cont.to_s
    puts ""
    textoBus = valorBuscarExpReg()
    rangoPa=""
    expresion=""
    ...
    puts"Los Valores entrenamiento: "+cont2.to_s
    # puts "Encontrados "+resultado.to_s
    # puts "Posiciones "+posiciones.to_s
    begin puntos = resultado.size.to_f rescue puntos = 0.0/0.0 end
    ...
    puts "Putuación: "+puntos.to_s
    return 0 if puntos.nan? or puntos.infinite?
    #sum_error += (puntos - funcionPrueba(textoBus,rangoCar)).abs
    sum_error += (puntos+tamExp.to_f)
    result=puntos.to_s
    $generaHtml<< '<tr><td>Valor Total de diferencia de Error: ' << sum_error.to_s << '</td></tr>' << '</table>'
    puts "Valor Total de diferencia de Error "+sum_error.to_s   

  #  $generaHtml<< '<tr><td>Diferencia de error: ' << sum_error.to_s << '</td></tr>'
...
def evalucion_R(candidata, codon, gramaticaE, max_pro)
  ...
  if (valorResult.strip.include?"(Cadena)") 
    ...
    $mejorEva.each do |valor|
      puts "Arreglo: "+valor.to_s+" valor "+valorResult.to_s
  ...
  puts "Canditatos: "+candidata[:fitness].to_s
  ...
  if (valorResult=="") 
    ...
    if (candidata[:fitness].to_f > 0) 
      ...
      #$generaHtml << '<tr><td>Solución: ' << mejorEva[:fitness].to_s << '</td></tr>'
      $generaHtml2 << '<td>' << valorResult.to_s << '</td></tr>'
    end
  end
  puts "Canditatos 1: "+candidata[:program].to_s
end
...
def busqueda(max_gens, poblacion, codon, num_bits, pcruce, gramaticaE, max_pro )
    ...
    puts " "
    puts " "   
    puts " > gen=#{gen}, f=#{$mejorEva[:fitness]}, Cromosoma=#{$mejorEva[:valEntero]}"
    puts " "
    # break if mejorEva[:fitness] == 0.0
  end
  return $mejorEva
end
...
Benchmark.bm (7) do |x|
    ...
    puts " "
    puts "Solución: f=#{$mejorEva[:fitness]}"
    puts "s=/#{$mejorEva[:program]}/"
    puts " "
    puts "Rendimiento del programa"
    ...
    #poblacion.times do |item|
      #$generaHtml2 << "<tr><td>Solución: #{item} </td></tr>"
     # $generaHtml2 << '<tr><td>Expresion Regular Generada: ' << mejorEva[:program].to_s << '</td></tr>'
    #end
